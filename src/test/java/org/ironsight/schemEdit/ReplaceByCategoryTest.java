 ...
 *
 * Uses {@code jerusalem_tower_pretty_I.schem} which contains:
 *   - minecraft:red_sandstone                (bare block)
 *   - minecraft:red_sandstone_stairs[...]    (multiple blockstate variants)
 ...
 *   - minecraft:acacia_planks               (different category: acacia)
 *   - minecraft:ladder[...]                 (no category — maps to itself)
 *   - minecraft:cut_sandstone               (category: cut_sandstone — no stairs/slab
 *                                            equivalent in red_sandstone)
 *   - minecraft:air
 */
public class ReplaceByCategoryTest {
    ...
    public void blockstatePropertiesArePreserved() throws Exception {
        ...
        boolean hasStairsWithState = palette.stream()
                .anyMatch(b -> b.startsWith("minecraft:sandstone_stairs[")
                        && b.contains("facing="));
    ...
    /**
     * red_sandstone → oak: oak has no wall, so red_sandstone_wall should remain
     * unchanged. oak_stairs and oak_slab exist, so those should be replaced.
     */
    @Test
    public void noEquivalentInTarget_blockKeptUnchanged() throws Exception {
        Schematic source = BlockReplacer.load(TOWER);
        Schematic result = BlockReplacer.replaceByCategory(source,
                Map.of("red_sandstone", "oak_wood"));

        Set<String> palette = BlockReplacer.getPalette(result);

        // oak_wood has no wall → red_sandstone_wall must stay
        assertTrue("red_sandstone_wall should remain (oak_wood has no wall)",
                palette.stream().anyMatch(b -> b.contains("red_sandstone_wall")));

        // oak_wood has stairs → red_sandstone_stairs should be replaced
        assertFalse("red_sandstone_stairs should be replaced",
                palette.stream().anyMatch(b -> b.contains("red_sandstone_stairs")));
        assertTrue("oak_stairs should appear",
                palette.stream().anyMatch(b -> b.contains("oak_stairs")));
    }
    ...
    public void mapCategoryToItself_producesIdenticalPalette() throws Exception {
        Schematic source = BlockReplacer.load(TOWER);
        Schematic result = BlockReplacer.replaceByCategory(source,
                Map.of("red_sandstone", "red_sandstone"));
    ...
    /**
     * Replaces bricks, brick_slab, brick_stairs with their cobblestone equivalents
     * using a programmatically built schematic.
     */
    @Test
    public void bricks_to_cobblestone() throws IOException {
        ...
        Schematic result = BlockReplacer.replaceByCategory(source,
                Map.of("bricks", "cobblestone"));

        Set<String> palette = BlockReplacer.getPalette(result);

        assertTrue("bricks → cobblestone",
                palette.contains("minecraft:cobblestone"));
        assertTrue("brick_slab → cobblestone_slab",
                palette.contains("minecraft:cobblestone_slab"));
        assertTrue("brick_stairs → cobblestone_stairs",
                palette.contains("minecraft:cobblestone_stairs"));
    }
}
