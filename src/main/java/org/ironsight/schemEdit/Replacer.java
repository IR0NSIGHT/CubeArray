...
public class Replacer {
     ...
     * <p>
     * For each entry {@code srcCat → tgtCat}, every block whose category is
     * {@code srcCat} is mapped to its equivalent block in {@code tgtCat}.
     * Blocks with no equivalent in the target category are silently skipped.
     * Identity mappings (where {@code srcCat == tgtCat}) are also skipped.
     * <p>
     * Blocks whose bare ID equals the source category ID (i.e. the category
     * base block) are mapped directly to the target category ID.
     *
    ...
    public String replaceBlockByCategory(String blockIn, String categoryOut) throws NotFoundExc {
        ...
        throw new NotFoundExc("No equivalent block found for '" + blockIn + "' in category '" + categoryOut + "'");
    }
    ...
    private String findBestRawMatch(String source, List<String> targetBlocks) {
        ...
        for (String target : targetBlocks) {
            ...
            boolean better = suffixLen > bestSuffixLen
                    || (suffixLen == bestSuffixLen && remPrefixLen > bestRemPrefixLen)
                    || (suffixLen == bestSuffixLen && remPrefixLen == bestRemPrefixLen && remSum < bestRemSum);
    ...
    private String findBestStrippedMatch(String srcStripped, List<String> targetBlocks) {
        ...
        for (String target : targetBlocks) {
            ...
            boolean better = baseSuffixLen > bestBaseLen
                    || (baseSuffixLen == bestBaseLen && boolToInt(sameSfx) > bestSfxMatch)
                    || (baseSuffixLen == bestBaseLen && boolToInt(sameSfx) == bestSfxMatch && boolToInt(samePref) > bestPrefMatch)
                    || (baseSuffixLen == bestBaseLen && boolToInt(sameSfx) == bestSfxMatch && boolToInt(samePref) == bestPrefMatch && prefCount < bestPrefCount);
    ...
    private static int boolToInt(boolean b) { return b ? 1 : 0; }
