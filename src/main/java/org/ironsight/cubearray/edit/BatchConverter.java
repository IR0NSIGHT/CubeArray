...
public class BatchConverter {
  ...
  /**
   * Converts a list of schematic files to Sponge v3 (.schem) format, writing a {@code
   * _sponge3.schem} file next to each original.
   *
   * <p>SchemConvert writes block data as plain bytes, but WorldPainter requires VarInt-encoded
   * block data. This method fixes the encoding after conversion so the output is readable by
   * WorldPainter for all palette sizes.
   *
   * <p>The original file is never deleted or modified. The output file is written atomically via a
   * temporary file so a failed conversion never leaves a partial result on disk.
   *
  ...
  public static ConversionResult convertToSponge3InPlace(List<Path> inputFiles) {
    ...
    for (Path inputPath : inputFiles) {
      ...
      try {
        ...
        Files.move(tempFile.toPath(), outputTarget.toPath(), StandardCopyOption.REPLACE_EXISTING);
  ...
  /**
   * Re-encodes the {@code Schematic/Blocks/Data} byte array in a Sponge v3 {@code .schem} file from
   * plain bytes to VarInts in-place.
   *
   * <p>SchemConvert writes {@code blockData[i] = (byte) paletteIndex}, which is only a valid VarInt
   * for indices 0–127. For indices ≥ 128 the high bit is set, which WorldPainter interprets as a
   * VarInt continuation byte, causing a "VarInt too big" error. This method rewrites the file with
   * properly encoded VarInts.
   */
  ...
  /**
   * Walks the raw NBT byte array to find the {@code Schematic/Blocks/Data} TAG_Byte_Array and
   * replaces each plain byte with a proper VarInt, returning the rewritten byte array.
   */
  ...
  /**
   * Recursively walks a TAG_Compound, copying tags verbatim until it finds the tag at {@code
   * path[depth]}. When the full path is reached the TAG_Byte_Array payload is re-encoded as
   * VarInts; all other tags are copied verbatim.
   */
  private static void walkCompound(
      DataInputStream in, DataOutputStream out, String[] path, int depth) throws IOException {
    while (true) {
  ...
  /**
   * Re-encodes a byte array of plain palette indices as VarInts. Each palette index is written as
   * 1–5 bytes following the standard Minecraft VarInt encoding (7 data bits per byte, MSB =
   * continuation).
   */
  ...
  private static void copyNbtPayload(DataInputStream in, DataOutputStream out, int type)
      throws IOException {
    switch (type) {
      case 1 -> {
        byte v = in.readByte();
      ...
      case 11 -> {
        int len = in.readInt();
        out.writeInt(len);
        byte[] d = in.readNBytes(len * 4);
        out.write(d);
      }
      case 12 -> {
        int len = in.readInt();
        out.writeInt(len);
        byte[] d = in.readNBytes(len * 8);
        out.write(d);
      }
  ...
  /**
   * Converts a list of schematic files to Sponge v3 (.schem) format, writing output files into
   * {@code outputDir} with the same base name as the input. Applies VarInt re-encoding after
   * conversion so WorldPainter can load files with palette sizes &gt; 127.
   *
   * @param inputFiles list of schematic files to convert
   ...
   */
  public static ConversionResult convertToSponge3(List<Path> inputFiles, File outputDir)
      throws IOException {
    if (!outputDir.exists())
