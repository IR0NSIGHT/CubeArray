...
/**
 * Modal dialog shown after the block-replacement mapping is confirmed.
 * Lets the user choose:
 * <ul>
 *   <li>A postfix string appended to each output filename (default: {@code "_replaced"})</li>
 *   <li>Optionally, a target folder to write all output files into (instead of each
 *       file's own directory)</li>
 * </ul>
 *
 * <p>A live example label shows {@code <firstBaseName><postfix>.schem} and updates
 * as the user types. If the folder checkbox is ticked, a {@link JFileChooser} is shown
 * immediately; if the user cancels the picker the checkbox is unticked again.
 *
...
public class OutputOptionsDialog extends JDialog {
     ...
     * @param outputFolder the folder to write outputs into, or {@code null} to use each
    ...
    private JPanel buildForm() {
        ...
        lbl.gridx = 0; lbl.gridy = 0; lbl.anchor = GridBagConstraints.WEST;
        lbl.insets = new Insets(0, 0, 6, 8);

        GridBagConstraints fld = new GridBagConstraints();
        fld.gridx = 1; fld.gridy = 0; fld.fill = GridBagConstraints.HORIZONTAL;
        fld.weightx = 1.0; fld.insets = new Insets(0, 0, 6, 4);

        GridBagConstraints btn = new GridBagConstraints();
        btn.gridx = 2; btn.gridy = 0; btn.anchor = GridBagConstraints.WEST;
        ...
        full.gridx = 0; full.gridy = 2; full.gridwidth = 2;
        full.anchor = GridBagConstraints.WEST; full.insets = new Insets(0, 0, 4, 0);
        panel.add(folderCheckBox, full);

        // Row 3 — folder path label (spans both columns)
        full.gridy = 3; full.insets = new Insets(0, 20, 0, 0);
        ...
        postfixField.getDocument().addDocumentListener(new DocumentListener() {
            public void insertUpdate(DocumentEvent e)  { updateExample(); }
            public void removeUpdate(DocumentEvent e)  { updateExample(); }
            public void changedUpdate(DocumentEvent e) { }
        });

        // Folder checkbox — open picker when ticked, clear when unticked
        folderCheckBox.addActionListener(e -> {
    ...
    private void updateExample() {
        if (firstFile == null) { exampleLabel.setText(""); return; }
    ...
    private File pickFolder() {
        JFileChooser chooser = new JFileChooser(
                firstFile != null ? firstFile.getParentFile() : null);
     ...
     * @param firstFile the first source file — used to build the live example; may be
     *                  {@code null} if no files are selected
    ...
    public static Optional<Options> show(Component parent, File firstFile) {
        ...
        return Optional.of(new Options(dialog.postfixField.getText(), dialog.isPrefix, dialog.chosenFolder));
    }
}
