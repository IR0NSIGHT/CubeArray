...
public class AutoCompletion {
  ...
  /**
   * Rebuilds the combo model to only contain items whose name contains {@code query}
   * (case-insensitive). Opens or closes the popup accordingly.
   */
  ...
  /**
   * Writes the currently highlighted item into the text field and closes the popup. Restores the
   * full item model so the next search starts from scratch.
   */
  ...
  public static void main(String[] args) throws Exception {
    String[] blocks =
        org.ironsight.schemEdit.BlockListUtil.loadBlockData().keySet().stream()
            .sorted()
            .toArray(String[]::new);
