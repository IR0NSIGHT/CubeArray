 ...
 * <ul>
 *   <li><b>Blocks</b> — one row per unique block type. Blockstate properties are
 *       transplanted automatically via {@link BlockListUtil#toLegalBlockState}.</li>
 *   <li><b>Details</b> — one row per exact block-state variant. No transplanting.</li>
 * </ul>
 *
...
public class BlockReplacerDialog extends JDialog {
    ...
    private enum Mode { BLOCKS, DETAILS }
    ...
    /**
     * Result returned by {@link #show}: a direct {@code fullId → fullId} map
     * for {@link BlockReplacer#replace}.
     */
    ...
    private static Icon unknownIcon() {
        ...
        Icon icon = new ImageIcon(new ImageIcon(url).getImage().getScaledInstance(32, 32, Image.SCALE_SMOOTH));
    ...
    private static Icon loadIcon(String bareId) {
        ...
        URL url = BlockReplacerDialog.class.getClassLoader().getResource("icons/minecraft_" + iconKey + ".png");
        if (url != null) {
            Icon icon = new ImageIcon(new ImageIcon(url).getImage().getScaledInstance(32, 32, Image.SCALE_SMOOTH));
    ...
    private static Icon loadIconSmall(String bareId) {
        ...
        URL url = BlockReplacerDialog.class.getClassLoader().getResource("icons/minecraft_" + iconKey + ".png");
        if (url != null) {
            Icon icon = new ImageIcon(new ImageIcon(url).getImage().getScaledInstance(20, 20, Image.SCALE_SMOOTH));
    ...
    /**
     * Combo box editor for category mode. The editor panel shows the category
     * name in an editable text field on the left and a live icon strip on the right,
     * both updating whenever {@link #setItem} is called.
     */
    private static class CategoryComboEditor extends BasicComboBoxEditor {
        ...
        public JTextField getTextField() { return editor; }
        ...
        private void rebuildStrip(String category) {
            ...
            blocks.stream().limit(MAX_CATEGORY_ICONS).forEach(blockId -> {
    ...
    /**
     * Renderer for the category dropdown: shows the category name on the left
     * followed by small icons of all blocks belonging to that category (up to
     * {@value #MAX_CATEGORY_ICONS}).
     */
    ...
    private final Map<String, List<String>> categoryToBlocks;     // category → distinct stripped block IDs in palette
    private final Map<String, List<String>> allCategoryBlocks;    // category → all blocks in that category (full data)
    private final ListCellRenderer<Object>  categoryIconRenderer; // renderer showing icon strip per category row
    ...
    private BlockReplacerDialog(Frame owner, Set<String> palette, Set<String> availableBlocks) {
        ...
        this.realCategories = categoryEntries.stream()
                .map(CategoryEntry::id)
                .collect(Collectors.toSet());

        this.replacer = new Replacer(categoryEntries);

        // --- Blocks-mode data ---
        Map<String, List<String>> unsorted = palette.stream()
                .sorted()
                .collect(Collectors.groupingBy(BlockReplacer::stripBlockId, LinkedHashMap::new, Collectors.toList()));
        this.groups = unsorted.entrySet().stream()
                .sorted(java.util.Comparator.<Map.Entry<String, List<String>>, String>comparing(
                                e -> blockToCategory.getOrDefault(e.getKey(), e.getKey()))
                        .thenComparing(Map.Entry::getKey))
                .toList();
        this.choices = availableBlocks.stream().sorted().toArray(String[]::new);
        this.displayChoices = java.util.stream.Stream.concat(
        ...
        this.detailChoices = java.util.stream.Stream.concat(
                        availableBlocks.stream().sorted(),
                        palette.stream().sorted())
                .distinct().sorted().toArray(String[]::new);

        // All known category IDs from JSON
        this.categoryChoices = categoryEntries.stream()
                .map(CategoryEntry::id)
                .distinct().sorted().toArray(String[]::new);
    ...
    private JScrollPane buildMappingPanel() {
        ...
        srcC.gridx = 0; srcC.gridy = 0; srcC.anchor = GridBagConstraints.WEST;
        srcC.insets = new Insets(2, 4, 2, 8); srcC.weightx = 0.5; srcC.fill = GridBagConstraints.HORIZONTAL;

        GridBagConstraints destC = new GridBagConstraints();
        destC.gridx = 1; destC.gridy = 0; destC.anchor = GridBagConstraints.WEST;
        destC.insets = new Insets(2, 0, 2, 4); destC.weightx = 0.5; destC.fill = GridBagConstraints.HORIZONTAL;

        rows.add(bold("Source block"), srcC);
        rows.add(bold("Replace with"), destC);
        srcC.gridy++; destC.gridy++;
        ...
        int rowCount = switch (mode) {
            ...
            case DETAILS -> (int) variants.stream()
    ...
    private void buildBlocksRows(JPanel rows, GridBagConstraints srcC, GridBagConstraints destC) {
        ...
        Map<String, Long> catCount = groups.stream()
                .filter(g -> matchingGroups.contains(g.getKey()))
                .map(g -> blockToCategory.getOrDefault(g.getKey(), g.getKey()))
                .filter(realCategories::contains)
                .collect(Collectors.groupingBy(c -> c, Collectors.counting()));
        Set<String> groupedCategories = catCount.entrySet().stream()
        ...
        for (int gi = 0; gi < groups.size(); gi++) {
            ...
            if (!Objects.equals(category, currentCategory)) {
                ...
                if (groupedCategories.contains(category) && visibleCategories.contains(category)) {
                    ...
                    AutoCompletion.enable(catCombo, catEditor.getTextField(),
                            () -> {
            ...
            String label = varList.size() > 1
                    ? displayName + "  (" + varList.size() + " variants)"
                    : displayName;
    ...
    private void buildDetailsRows(JPanel rows, GridBagConstraints srcC, GridBagConstraints destC) {
        ...
        for (int vi = 0; vi < variants.size(); vi++) {
            ...
            JLabel srcLabel = new JLabel(variant, loadIcon(BlockReplacer.stripBlockId(variant)), JLabel.LEADING);
            srcLabel.setIconTextGap(6);
            rows.add(srcLabel, srcC);
    ...
    private JComboBox<String> makeCombo(String[] choices, String sourceKey) {
        ...
        AutoCompletion.enable(combo, iconEditor.getTextField(),
                () -> {
    ...
    private ReplaceResult buildResult() {
        Map<String, String> result = switch (mode) {
    ...
    /**
     * Blocks mode: transplant each source variant's {@code [...]} onto the chosen
     * replacement, then validate via {@link BlockListUtil#toLegalBlockState}.
     */
    private Map<String, String> buildResultBlocks() {
        ...
        for (int ci = 0; ci < combos.size(); ci++) {
            ...
            String dest = Arrays.stream(choices)
                    ...
                    .orElseGet(() -> groups.stream()
                            ...
                            .map(id -> { int b = id.indexOf('['); return b >= 0 ? id.substring(0, b) : id; })
                            .findFirst()
                            .orElse(destDisplay));
    ...
    /**
     * Details mode: map each exact source variant verbatim to the chosen replacement.
     * No blockstate transplanting — what the user picks is what goes in.
     */
    ...
    public static Optional<ReplaceResult> show(
        ...
        Frame owner = parent == null ? null
                : (parent instanceof Frame f ? f : (Frame) SwingUtilities.getAncestorOfClass(Frame.class, parent));
