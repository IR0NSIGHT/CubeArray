...
public class GlUtils {
    public static int bind1DTexturePalette(Vector4f[] inputData, String textureName, int textureId, int shaderProgram) {
        ...
        glBindTexture(GL_TEXTURE_2D, colorPaletteTexID);
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, inputData.length, 1, 0, GL_RGBA, GL_FLOAT, colorBuffer);
    ...
    public static int bind1DTexturePalette(Vector3f[] inputData, String textureName, int textureId, int shaderProgram) {
        ...
        glBindTexture(GL_TEXTURE_2D, colorPaletteTexID);
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, inputData.length, 1, 0, GL_RGB, GL_FLOAT, colorBuffer);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
