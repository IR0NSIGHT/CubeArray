...
public class InstancedCubes {
  ...
  private void loop() {
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      if (rotateCameraByMouse) {
        ...
        cameraState =
            new CameraState(
                cameraState.target(), yaw, pitch, cameraState.radius // new radius
                );
    ...
    try {
      out =
          ResourceUtils.getScreenshotPath()
              ...
              .toFile();
      if (out.exists()) return; // pointless to screenshot the sam ething twice.
  ...
  private CameraState zoom(CameraState cameraState, float factor) {
    ...
    return new CameraState(
        cameraState.target(), cameraState.yaw(), cameraState.pitch(), newRadius // new radius
        );
  }
  ...
  public record CameraState(Vector3f target, float yaw, float pitch, float radius) {
